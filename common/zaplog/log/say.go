...
func ErrorIf(err error, fields ...zap.Field) {
	if err == nil {
		return
	}
	defer zap.L().Sync()
	fds := []zap.Field{zap.Error(err)}
	fds = append(fds, fields...)
	zap.L().Error("error", fds...)
}

func PanicIfErr(err error, fields ...zap.Field) {
	if err == nil {
		return
	}
	defer zap.L().Sync()
