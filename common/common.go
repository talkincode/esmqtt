...
func WriteFile(src []byte, to string, mode os.FileMode) error {
	// from是文件，那么创建to的文件夹
	p := filepath.Dir(to)
	if _, e := os.Stat(p); e != nil {
		if e = os.MkdirAll(p, mode); e != nil {
...
func CreateTmpFile(data []byte) (*os.File, error) {
	tmpFile, err := os.CreateTemp(os.TempDir(), "peagpt-")
	if err != nil {
...
func WriteJson(v interface{}, filepath string) error {
...
func WriteText(v string, filepath string) error {
	fs, err := os.Create(filepath)
	if err != nil {
		return err
	}
	defer fs.Close()
	_, err = fs.Write([]byte(v))
	return err
}
...
func ReadJsonRd(rd io.Reader, v interface{}) error {
...
func ReadJsonl(rd io.Reader) ([]map[string]interface{}, error) {
	var result []map[string]interface{}
	scanner := bufio.NewScanner(rd)

	// Set the split function for the scanning operation.
	buf := make([]byte, 0, 64*1024)
	scanner.Buffer(buf, 10*1024*1024) // Set the maximum size to 10MB
...
func TrimPrefixAllSpace(s string) string {
	return strings.TrimLeftFunc(s, unicode.IsSpace)
}

func SafeTruncate(str string, length int) string {
	runes := []rune(str)
	if len(runes) > length {
		runes = runes[:length-3]
...
func DownloadFile(url, filepatch string) error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("unexpected status code while downloading file: %d", resp.StatusCode)
	}

	tmpFile, err := os.Create(filepatch)
	if err != nil {
		return err
	}
	defer tmpFile.Close()
...
func MapsToStructs(m []map[string]interface{}, obj interface{}) error {
...
func RandomCardString() string {
	mathrand.Seed(time.Now().UnixNano())
	num1 := mathrand.Intn(900) + 100 // Generate a random number between 100 and 999
	num2 := mathrand.Intn(900) + 100 // Generate a random number between 100 and 999
	return fmt.Sprintf("%d-%d", num1, num2)
}

func GetValueOrDefaultString(value, defaultValue string) string {
	if value != "" {
		return value
	}
	return defaultValue
